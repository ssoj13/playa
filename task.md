...
# Task:
  ...
  2. Как у нас хранятся слои в Comp? просто Vec<Tuple(Uuid, Attrs)> или у нас это отдельный тип с сериализацией? Ты бы как сделал? Может сделать отдельный файл Layer.rs и там определить и чтобы Comp оперировала с данными оттуда? Можно инвалидировать атрибуты per-layer и прочие другие штуки. Предложи солюшны.
  3. Рассмотреть возможность добавления нескольких слоёв на один трек в timeline: видимо перейти от Vec<Layer> на Vec<Vec<Layer, ..>, ..>? Предложи хорошие варианты.
  4. Изучи вопрос добавления Node editor. Какие крейты можно использовать? Для egui есть несколько с нодами, изучи вопрос. Каждый комп можно представить просто как ноду с кучей инпутов, куда воткнуты другие компы. Идеальная страктура для node network.
