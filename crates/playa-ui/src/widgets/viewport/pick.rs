...
use playa_engine::entities::comp_node::CompNode;
use playa_engine::entities::node_kind::NodeKind;
use playa_engine::entities::space;
use playa_engine::entities::transform::build_inverse_transform;
...
pub fn pick_layer_at(
    ...
    let Some(image_pos) = viewport_state.screen_to_image(eframe::egui::vec2(local.x, local.y))
    else {
    ...
    debug!(
        "[pick] image={:?} comp={:?} zoom={} pan={:?}",
        image_pos, comp_pos, viewport_state.zoom, viewport_state.pan
    );
    ...
    for (i, layer) in comp.layers.iter().enumerate() {
        ...
        if frame_idx < play_start || frame_idx > play_end {
            debug!(
                "[pick] layer[{}] = {} SKIP (frame {} not in {}..{})",
                i, name, frame_idx, play_start, play_end
            );
            continue;
        }
