...
impl Frame {
    ...
    pub fn load_header(&self) -> Result<usize, FrameError> {
        ...
        let attrs = super::loader::Loader::header(&path)?;
        ...
        trace!("Loaded header: {}x{}", width, height);
        Ok(0)
    }
    ...
    pub fn load(&self) -> Result<usize, FrameError> {
        ...
        match super::loader::Loader::load(&path).and_then(|src| self.ingest_loaded_frame(src)) {
            Ok(mem_size) => {
    ...
    fn ingest_loaded_frame(&self, src: Frame) -> Result<usize, FrameError> {
        let buffer = src.buffer();
        let pixel_format = src.pixel_format();
        let (width, height) = src.resolution();
