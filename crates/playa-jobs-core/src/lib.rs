...
pub use queue::{JobFilter, JobQueue, JobQueueConfig, JobStats};
pub use settings::JobsSettings;

// Re-export the event bus so downstream consumers don't need to add
// `playa-events` as a separate dep — `playa_jobs_core::EventBus` is the
// canonical entry point for subscribing to JobEvents.
pub use playa_events::{EventBus, EventEmitter};
