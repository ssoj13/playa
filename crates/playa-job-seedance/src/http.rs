...
//!
//! Splitting the trait out lets tests run the full provider state machine
//! against a scripted in-memory mock without an internet connection or a real
//! API key.
...
/// provider wraps them into [`playa_jobs::JobError::Provider`] /
/// [`playa_jobs::JobError::Io`] depending on which call site failed.
...
impl UreqFalHttp {
    pub fn new() -> Self {
        // 5-min connect/read timeout: long enough for fal's longest queue
        // wait between requests, short enough that a stuck connection does
        // not pin a worker indefinitely (cancel token still wins via the
        // 5 s poll cadence in the provider).
