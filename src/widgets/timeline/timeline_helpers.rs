...
impl LayerTool {
    ...
    pub(super) fn to_drag_state(&self, layer_idx: usize, attrs: &crate::entities::Attrs, drag_start_pos: Pos2) -> GlobalDragState {
        match self {
...
pub(super) fn detect_layer_tool(hover_pos: Pos2, bar_rect: Rect, edge_threshold: f32) -> Option<LayerTool> {
...
pub(super) fn frame_to_screen_x(frame: f32, timeline_rect_min_x: f32, config: &TimelineConfig, state: &TimelineState) -> f32 {
    timeline_rect_min_x + (frame - state.pan_offset) * config.pixels_per_frame * state.zoom
}

pub(super) fn screen_x_to_frame(x: f32, timeline_rect_min_x: f32, config: &TimelineConfig, state: &TimelineState) -> f32 {
