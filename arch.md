...
Comp адаптируется к длине вложенных композиций, учитывает их trim_in / trim_out.

Нужно подумать, можно ли как-то сделать Comp или блок загрузки media трейтом?
По умолчанию Comp - просто контейнер, он грузит файлы только в file mode.
Как лучше сделать, может есть идеи.
...
### Timeline
  - Основной контроллер для работы с Comp
  - Состоит из двух окон находящихся в доке со слайдером. Окна должны работать и самостоятельно.
    - левое, меньшее (где-то пикселей 400)
      - playback controls, loop checkbox, zoom slider
      - контролами для каждого слоя: Visibility, Solo, Transparency, Overlay_mode
    - правое, большее:
      - time ruler - линейка с кадрами и динамической отрисовкой рИсок на каждом N кадре: каждом 5, 10, 100 в зависимости от масштаба
      - time slider - вертикальная полоса проходяшая через весь размер этого окна по вертикали, показывает текущий кадр. Левой мышью можно за него хватать.

  - Механизм Drag'n'drop:
    - Каждый Comp должен иметь функцию отдающую координаты нужных элементов в виде массива или HashMap:
      - словарь в котором в разных разделах лежат clip edges, clip bounds - элементы которые могут реагировать на движения мыши поверх них или на хватание.
    - При движении мыши мы постоянно проверяем где она находится и подсвечиваем разные элементы - клипы, их границы, чтобы показать что сработает когда пользователь нажмёт или потащит этот элемент.
    - Эти элементы сортируются по дальности от мыши и захватываются с некой дистанцией, скажем 5 пикселей. Если элемент "захвачен" - то при нажатии должен использоваться именно он. Тут нужна подсказка как правильно сделать.
    - Timeline по сути - это Comp Display. Окно, показывающее состояние Comp и его внутренности в виде слоёв, похоже на After Effects.
    - Drag'n'drop:
      - нужно уметь drag'n'drop из окна Project. Там нужно хватать предмет и бросать его на Timeline. Он должен добавляться в текущий Active comp.
      - нужно уметь двигать клип по таймлайну влево-вправо по времени.
      - В какой-то степени Timeline - это почти как Comp, поэтому у него тоже есть in/out, trim_in/trim_out в атрибутах (play range) и функции конверсии времени из Timeline во время клипов и обратно.


### Viewport
  - Принимает &Frame и показывает его в окне с помощью шейдеров в разных цетовых глубинах (u8, f16, f32) и с разным Tonemapping.
  - Три шейдера встроены прямо в программу, дополнительно сканируется каталог shaders (если существует) и ищутся и грузятся другие шейдера, обновляя combobox на вьюпорте.
  - Может панорамировать и зумить окно мышью.
  - Обрабатывает мышь и кнопки
  - Имеет функции отображения один к одному и Fit All - в размер окна.
...
└───────┬───────────┘   (frame count, dimensions, fps)
        ...
        ▼
┌────────────────────┐
│ Project.set_active │ → activate comp, emit CurrentFrameChanged
└───────┬────────────┘
        │
        ▼
┌───────────────────┐
