...
**Пути в столбце «Файл»:** префиксы `core/` … `entities/` — это **`crates/playa-engine/src/`**;  
`widgets/` … `dialogs/` — **`crates/playa-ui/src/`**.
...
типа есть `*_SCHEMA` в `entities/attr_schemas.rs` (**crate `playa-engine`**),
...
// → обработчик в `crates/playa-app` (модуль `main_events`):
...
`entities/traits.rs` (**`crates/playa-engine/src/entities/traits.rs`**) задаёт интерфейсы (`FrameCache`, `WorkerPool`,
`CacheStrategy`), которые инфраструктура должна давать модели **entities**. Конкретные реализации
(`GlobalFrameCache`, `Workers`) лежат в **`crates/playa-engine/src/core/`**. Оболочка (`playa-app`) композирует **`playa-ui`**
и маршрутизирует события / мутации `Project`; узлы через **`ComputeContext`**.

```
playa-app (+ playa-ui)  ──→  PlayaApp / EventBus / UI
                                       │
                                       ▼
                          playa-engine: core ──→ entities (через трейты в ComputeContext)
```
...
timeline state, viewport state). События в `core/layout_events.rs` (**`playa-engine`**):
...
если не передан **`--features` / `-f`** — тогда используется **`cargo build -p playa`** с **`--features`**
(**`DEVELOP.md`**). Тонкий **`build.rs`** триггерит только `rerun-if-changed`; FFmpeg и прочие нативы — через Cargo + **vcpkg**.

```
python bootstrap.py build
python bootstrap.py build -d
python bootstrap.py build -f profiler # пример Cargo feature `profiler`
...
1. `crates/playa-engine/src/entities/foo_node.rs` со структом + `impl Node`.
2. Вариант в **`playa-engine`** `enum NodeKind`.
3. Схема в **`playa-engine`** `entities/attr_schemas.rs` (композировать общие `IDENTITY`, `TIMING`, `TRANSFORM`).
...
3. Обработка: `if let Some(e) = downcast_event::<MyEvent>(&event)` в
   `crates/playa-app/src/app/events.rs::handle_events` или **`main_events`** `handle_app_event`.
4. Если событие меняет проект — внутри `project.modify_comp` чтобы
   автоинвалидация сработала.

### Добавление эффекта

1. `crates/playa-engine/src/entities/effects/foo.rs` с функцией `apply(&Frame, &Effect) → Frame`.
2. Вариант в **`playa-engine`** `EffectType`.
3. Схема **`FX_FOO_SCHEMA`** в **`entities/attr_schemas.rs`** (поля с `FLAG_DAG | FLAG_DISPLAY | FLAG_KEYABLE`).
