...
```
src/
├── main.rs             # бинарник: ffmpeg::init → cli → log → run_app
├── lib.rs              # либо-крейт: pub mod core/app/entities/widgets/...
├── runner.rs           # eframe::run_native + восстановление состояния
├── cli.rs              # clap::Args + версия с бэкенд-инфо
├── config.rs           # PathConfig (CLI/ENV/dirs-next), цвета темы, дефолты
├── help.rs             # F1-оверлей
├── shell.rs            # OS-интеграция (drag-drop)
├── ui.rs               # композиция верхнего меню/панелей
├── utils.rs            # парсинг последовательностей, расширения медиа
├── main_events.rs      # центральный обработчик событий (AppEventContext)
│
├── app/                # PlayaApp + eframe::App impl
│   ├── mod.rs          # PlayaApp struct (47+ полей), Default, build_dock_state
│   ├── run.rs          # eframe::App::update() — главный кадр
│   ├── events.rs       # handle_events: poll EventBus → handle_app_event
│   ├── tabs.rs         # DockTabs: TabViewer для egui_dock
│   ├── layout.rs       # named layouts, sync_dock_tabs_visibility
│   ├── project_io.rs   # save/load Project, load_sequences
│   └── api.rs          # запуск REST-сервера, обработка ApiCommand
│
├── core/               # «движок» — независим от UI
│   ├── event_bus.rs        # EventBus (sub/emit/poll), Event, BoxedEvent
│   ├── player.rs           # Player: state в Attrs, JKL шаттл, FPS пресеты
│   ├── player_events.rs    # SetFrame, TogglePlayPause, JogForward/Backward, ...
│   ├── workers.rs          # Workers: work-stealing deques + epoch
│   ├── cache_man.rs        # CacheManager: лимит памяти + epoch + dirty_repaint
│   ├── global_cache.rs     # GlobalFrameCache: nested HashMap + lru::LruCache
│   ├── debounced_preloader.rs  # 500ms задержка перед полным preload
│   └── layout_events.rs    # ResetLayout, LayoutSelected/Created/Deleted/...
│
├── entities/           # модель данных + бизнес-логика
│   ├── attrs.rs            # Attrs: HashMap<&str, AttrValue> + dirty + schema
│   ├── attr_schemas.rs     # FILE/COMP/LAYER/CAMERA/TEXT/PLAYER/PROJECT_SCHEMA
│   ├── keys.rs             # const-ключи: A_IN, A_OUT, A_FRAME, A_SPEED, ...
│   ├── traits.rs           # FrameCache, WorkerPool, CacheStrategy (DI-инверсия)
...
│   ├── comp_node.rs        # CompNode + Layer: composition с эффектами
│   ├── camera_node.rs      # CameraNode: fov, near/far, POI vs rotation
│   ├── text_node.rs        # TextNode: cosmic-text, общая FontSystem
...
│   ├── loader.rs           # классификация по расширению → exr/video/generic
│   ├── loader_video.rs     # FFmpeg метаданные и декод кадров
│   ├── project.rs          # Project: media pool + active + modify_comp pattern
│   └── effects/            # blur.rs, brightness.rs, hsv.rs (+ Effect, EffectType)
│
├── widgets/            # переиспользуемые egui-виджеты (общаются через EventBus)
│   ├── viewport/           # OpenGL viewport: renderer/shaders/gizmo/pick/tool
│   ├── timeline/           # таймлайн со слоями, work area, bookmarks
│   ├── project/            # медиа-панель (Project)
│   ├── node_editor/        # граф композиции (egui-snarl)
│   ├── ae/                 # Attribute Editor (универсальный для Attrs)
│   ├── status/             # статус-бар (FPS, имя файла, кэш, render time)
│   ├── file_dialogs.rs     # rfd-обёртки
...
│
└── server/             # REST API
    ├── mod.rs              # реэкспорт + перечисление эндпоинтов
...
этого они эмитят типизованные события в `EventBus` (`src/core/event_bus.rs`).
...
`Workers` (`src/core/workers.rs`) — пул потоков с **per-worker FIFO деками**
+ глобальным `Injector`:
...
`bootstrap.ps1` / `bootstrap.sh` → `cargo xtask`. Тонкий `build.rs` ничего не
делает — управление зависимостями и копирование DLL/so вынесены в xtask.

```
cargo xtask build [--release|--debug]   # сборка + post-build copy
cargo xtask post                        # копирование нативных либ
cargo xtask verify                      # проверка зависимостей
cargo xtask deploy [--install-dir P]    # установка в систему
cargo xtask pre                         # Linux: патч заголовков OpenEXR (для GCC 11+)
...
`VCPKGRS_TRIPLET`, `PKG_CONFIG_PATH`. Подробности — в `md/DEVELOP.md`.
...
- `serde(skip)` на runtime-полях; **обязательно** восстанавливать после
  десериализации (event_emitter, schemas, cache_manager) — см. `runner.rs`.
...
- **vcpkg** в `C:\vcpkg`, ENV: `$env:VCPKG_ROOT`. Перед сборкой OpenEXR-варианта
  активировать `vcvars64.bat` (через `Developer PowerShell for VS 2022`).
...
См. `md/DIAGRAMS.md` — Mermaid-диаграммы pipeline'а кадра, событий, кэша,
композиции и иерархии Node.

См. `md/WGPU.md` — отчёт о возможном порте на wasm32 + wgpu (заблокировано
FFmpeg, rouille, OpenGL renderer'ом, `sysinfo`).

См. `md/DEVELOP.md` — детали vcpkg, FFmpeg, кросс-платформенной сборки.

---

*Базис: rustdocs всех модулей `src/**/*.rs`, верифицировано против актуального
кода (commit `75240d7`). Если расходится с реальностью — правда в коде.*
